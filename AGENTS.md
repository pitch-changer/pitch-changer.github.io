...
- When adding localized FAQ or cross-links for tempo changes, take the BPM Changer extension URL from `~/projects/extensions/musiciantools-online/config/site_settings.yml` under `site.extension_promos.bpm_changer_extension.primary_url`. Take the localized Speed Changer tool URL from the same file's `site.base_url` plus `~/projects/extensions/musiciantools-online/config/locales/[LOCALE].yml` `routes.speed_changer`, with `/<locale>/` added for non-default locales.
